...
export default defineComponent({
  setup: () => {
    ...
    const realSelectionHandler = async function (){
      if(selection === document.getSelection().toString().trim())
        return;
      selection = document.getSelection().toString().trim()
      console.log(selection)
      if(selection.startsWith("l’") || selection.startsWith("d’"))
      ...
      if(selection.length > 0){
        ...
        const response = await axios.get("https://cors.bridged.cc/"+"https://www.collinsdictionary.com/dictionary/french-english/"+selection)
        // const start = `</h1>`
        ...
        if(!cancel && dictionaryEntry.value.length > "")
          dialog.value = true;
    ...
    onMounted(() =>{
      if(window.localStorage.getItem("currentbook"))
        text.value = window.localStorage.getItem("currentbook")
